...
- Любые git-операции в лаборатории
- Работа в кабинетах (`projects/`)
- Настройка атрибуции агентов (`git-authors.json`)
- Вопросы по git-структуре LabDoctorM (корень НЕ репозиторий)

## Границы применимости
- Корень `/root/LabDoctorM/` НЕ репозиторий (корневой `.git/` удалён 22.06.2026)
- Каждый кабинет `projects/<кабинет>/` — отдельный git-репозиторий со своим `.git/`
- `workspaces/` — НЕ репозитории (рабочие столы агентов)
- `git-authors.json` лежит в корне кабинета (НЕ в `.qwen/`)
- Атрибуция per-agent: единый git user запрещён, каждый коммит подписан агентом

## Чек-лист качества
- [ ] Работа внутри `projects/<кабинет>/` (НЕ в корне лаборатории)
- [ ] Коммит через `./bin/lab-commit.sh <агент> -m "..."` (атрибуция корректна)
- [ ] НЕ `git config user.name/email` вручную (перезатирание агентов)
- [ ] НЕ коммитить `.env`/ключи/cookies (pre-commit заблокирует)
- [ ] НЕ дублировать функционал (grep/glob перед кодом — PAT-004)
- [ ] Push через `git push origin <ветка>`, НЕ `--force` без согласования

## Анти-паттерны
- ❌ Коммит в корне `/root/LabDoctorM/` (там нет репозитория)
- ❌ Ручной `git config user.name/email` (ломает per-agent атрибуцию)
- ❌ Прямой коммит без lab-commit.sh (теряется подпись агента)
- ❌ `git push --force` без согласования ЗавЛаба
- ❌ Коммит `.env`/секретов (pre-commit блокирует, но лучше не стейджить)
- ❌ Создание `git-authors.json` самому (формат согласован, проси ЗавЛаба)
