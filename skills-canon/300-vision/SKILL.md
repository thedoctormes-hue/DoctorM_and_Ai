...
## Когда применять
- Анализ изображений и скриншотов (что на картинке)
- OCR и чтение текста/кода/диаграмм с изображений
- Описание и интерпретация визуального контента

Использует Cohere `command-a-plus-05-2026` (436K context, vision) для анализа изображений через `/v2/chat` API.

## Границы применимости
- 1 вызов Cohere за изображение; размер ≤10MB (base64)
- НЕ видео; НЕ подходит для длинных/анимированных форматов
- Требует `python3` + `base64` в PATH (metadata.openclaw.requires)
- Счётчик 300 вызовов (remaining); при исчерпании — `LIMIT_EXHAUSTED`
- Ротация 4 ключей (key1→key4→key1); на ошибке — следующий ключ, счётчик НЕ декремент

## Чек-лист качества
- [ ] Изображение получено (путь к файлу ИЛИ URL → curl скачать в /tmp)
- [ ] Вызов `vision.sh` ПОЗИЦИОННО: $1=путь, $2=prompt(опц), $3=lang(ru|en|auto)
- [ ] Ответ `VISION_SUCCESS` + текст; счётчик `remaining` -1 при успехе
- [ ] При сбое всех 4 ключей — проверить валидность ключей, `vision.sh --status`
- [ ] `total_fail` НЕ полагаться (всегда 0 по факту кода)

## Анти-паттерны
- ❌ Аргументы как флаги вместо позиционных ($1 путь обязателен)
- ❌ Превышение лимита (LIMIT_EXHAUSTED) — не дублировать вызов
- ❌ Повторный вызов вместо проверки ключей/статуса при сбое
- ❌ Файл >10MB или видео
- ❌ Полагаться на `total_fail` (всегда 0)
- ❌ Игнор ротации ключей (key_errors[] — реальный индикатор)
